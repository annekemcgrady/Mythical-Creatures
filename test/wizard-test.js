...
describe('Wizard', function() {
  ...
  it('should start rested', function() {
   var wizard = new Wizard({name: 'Jhun'}); // create wizard
    assert.equal(wizard.isRested, true); // assert isRested returns true
  });

  it ('should be able to cast spells', function() {
    var wizard = new Wizard({name: 'Jhun'});  // create wizard
    assert.equal(wizard.cast(), 'MAGIC BULLET'); // assert wizard.cast() returns 'MAGIC BULLET'
  });
  ...
  it ('should only be able to cast 3 spells', function() {
    ...
    assert.equal(wizard.isRested, true); // assert isRested is true
    wizard.cast();// cast()
    assert.equal(wizard.isRested, false); // assert isRested is false
    assert.equal(wizard.cast(), 'I SHALL NOT CAST');// assert cast() returns 'I SHALL NOT CAST!'
  });
});
