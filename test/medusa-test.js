...
describe('Medusa', function() {
  ...
  it('should unstone the the first victim and stone the 4th coming in', function() {
    var medusa = new Medusa('Meeka');// create medusa
    var victim1 = new Person('Lousia');// create first victim
    var victim2 = new Person('Bree'); // create second victim
    var victim3 = new Person('TayTay');  // create third victim
    var victim4 = new Person('Steve');// create fourth victim
   
    medusa.stare(victim1);// medusa stares at first victim
    assert.equal(medusa.statues[0].name, 'Lousia'); 
    assert.equal(victim1.stoned,true);// assert that the first victim is stoned
    medusa.stare(victim2);// medusa stares at second victim
    medusa.stare(victim3);// medusa stares at third victim
    medusa.stare(victim4); // medusa stares at fourth victim
    assert.equal(victim4.stoned, true);/// assert the fourth victim is stoned
    assert.equal(victim1.stoned, false);// assert that the first victim is not stoned
